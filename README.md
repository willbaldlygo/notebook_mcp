...
### 2. Authentication
You have two options to authenticate. The **Direct Login** method is recommended as it's more durable.

#### Option A: Direct Login (Recommended)
This runs the browser visibly so you can log in once. The session is saved to `chrome_profile_mcp/`.

1.  Run the client in visible mode:
    ```bash
    python3 notebooklm_client.py --url "https://notebooklm.google.com" --question "init" --visible
    ```
2.  A Chromium window will open.
3.  **Log in to Google** inside that window.
4.  Once you see your notebooks, close the window.
5.  Future runs (headless) will use this saved profile.

#### Option B: Manual Cookie Export (Fallback)
If Option A fails (e.g. headless detection), export your cookies from Chrome:
1.  Log in to [NotebookLM](https://notebooklm.google.com/).
2.  Open **DevTools** -> **Application** -> **Cookies**.
3.  Copy all cookies and paste them into `notebooklm_cookies.json` in this directory.
